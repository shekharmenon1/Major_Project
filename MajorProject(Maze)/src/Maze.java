...
public class Maze extends Canvas
{
    ...
    Coordinate[] coordinates_to_draw; //initialize a array containing the coordinates to draw
    ...
    public void addcoordinate(Coordinate C){
        //coordinates_to_draw = Arrays.copyOf(coordinates_to_draw, coordinates_to_draw.length + 1);
        //coordinates_to_draw[coordinates_to_draw.length-1] = C;
        System.out.println("Coordinate x: "+C.getx());
    }
    //paint a creating maze
    public void paint_create(Graphics g, JButton[] Buttons, int rownumber){
        for (int i = 0;i<Buttons.length;i=i+1){
            if (Buttons[i].isSelected() == true && Buttons[i+1].isSelected() == true){
                g.drawLine((Buttons[i].getX()*50)+50, Buttons[i].getY()*50, (Buttons[i+1].getX()*50)+50, Buttons[i+1].getY()*50);
            }
            if (Buttons[i].isSelected() == true && Buttons[i+rownumber].isSelected() == true){
                g.drawLine((Buttons[i].getX()*50)+50, Buttons[i].getY()*50, (Buttons[i+rownumber].getX()*50)+50, Buttons[i+rownumber].getY()*50);
            }
        }
    }
}